...
import { AdjustmentsVerticalIcon, ChevronDownIcon, ChevronLeftIcon, ChevronRightIcon, ChevronUpIcon, CircleStackIcon, PhotoIcon, ShieldCheckIcon } from "@heroicons/react/24/outline";
...
const SidebarNavItem: React.FC<{
  ...
  const iconClass = twMerge(
    "size-4",
    !isSidebarOpen && "md:block md:size-5 md:self-center",
  );
  ...
  if (isDirect) {
    return <a className={itemClass} href={url} title={label}><Icon className={iconClass} /><span className={labelClass}>{label}</span></a>
  } else {
    return <Link className={itemClass} to={url} title={label}><Icon className={iconClass} /><span className={labelClass}>{label}</span></Link>
  }
}
...
const SidebarNav: React.FC = function ({ }) {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (isSidebarNavOpen && dropdownRef.current && !dropdownRef.current.contains(event.target)) {
  ...
  const navItems = (
    <>
      <SidebarNavItem navItem={SidebarActiveNav.IMAGES} url={URLS.IMAGE_LIST()} sidebarActiveNav={sidebarActiveNav} isSidebarOpen={isSidebarOpen} />
      <SidebarNavItem navItem={SidebarActiveNav.DATASETS} url={URLS.DATASET_LIST()} sidebarActiveNav={sidebarActiveNav} isSidebarOpen={isSidebarOpen} />
      <SidebarNavItem navItem={SidebarActiveNav.SETTINGS} url={URLS.SETTINGS} sidebarActiveNav={sidebarActiveNav} isSidebarOpen={isSidebarOpen} />
      {user.isStaff && urls.adminBackend && <SidebarNavItem navItem={SidebarActiveNav.ADMIN} navLabels={navLabels} url={urls.adminBackend} isDirect={true} sidebarActiveNav={sidebarActiveNav} isSidebarOpen={isSidebarOpen} />}
    </>
  )
  ...
  return (
    <div ref={dropdownRef} className="relative w-full">
      <button
        type="button"
        onClick={() => { setIsSidebarNavOpen(!isSidebarNavOpen) }}
        className={twMerge(
          ...
          isSidebarNavOpen ? "!rounded-b-none border-light-300 dark:border-dark-300 !border-b-transparent dark:!border-b-transparent" : "",
          !isSidebarOpen && "md:hidden",
        )}
      >
        {activeLabel}<ChevronDownIcon className="size-4" />
      </button>
    ...
    <div className={twMerge(
      ...
      <div className={twMerge(
        ...
        <div className={twMerge(
          ...
          isSidebarOpen ? "flex-row" : "md:flex-col",
        )}>
          <button type="button" onClick={toggleSidebar} className={twMerge(
      ...
      {isSidebarOpen && (
        <div className="relative w-full flex-1">
          <div className="absolute left-0 top-0 right-0 bottom-0 overflow-y-auto min-w-sidebar-open">
            {children}
          </div>
        </div>
      )}
