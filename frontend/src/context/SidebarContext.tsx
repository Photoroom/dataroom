...
export function SidebarProvider({
  ...
  const [isSidebarOpen, setIsSidebarOpen] = useState<boolean>(() => {
    ...
    const sidebarCookie = cookies.find((cookie) =>
      cookie.trim().startsWith("sidebar="),
    );
    return sidebarCookie
      ? sidebarCookie.split("=")[1].trim() === "open"
      : false;
  });
  const [isSidebarNavOpen, setIsSidebarNavOpen] = useState<boolean>(false);
  const [sidebarActiveNav, setSidebarActiveNav] =
    useState<SidebarActiveNav>(initialActiveNav);
