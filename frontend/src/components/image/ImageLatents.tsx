          ...
          image ? (
            <>
              {image.latents?.map((latent) => (
                <a key={latent.latent_type} href={latent.file_direct_url} target="_blank" className="group flex flex-row items-center gap-2 text-sm break-all">
                  {latent.is_mask ? <Square3Stack3DIcon className="size-6" /> : <CodeBracketSquareIcon className="size-6" />}
                  <span className="group-hover:underline">{latent.latent_type}</span>
                </a>
              ))}
              {image.latents?.length === 0 && (
                <p className="opacity-50">No latents</p>
              )}
            </>
          ) : (
            <>
