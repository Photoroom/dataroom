...
export const ImageSimilarPreview: React.FC<ImageSimilarProps> = ({ imageId }) => {
  ...
  const {
    ...
    isError: isErrorSimilarImages,
  } = useImagesSimilarList(imageId, {
      ...
      <div className="flex flex-col gap-4">
        <div className="grid grid-cols-2 gap-2 mb-4">
          {isLoadingSimilarImages ? (
            ...
            <p className="col-span-2 text-sm text-red-800 dark:text-red-400">
              Error fetching similar images! Does the image have an embedding?
            </p>
          ) : (
            similarImages?.map(similar => <Image key={similar.id} image={similar} />)
          )}
        </div>
        <button className="btn btn-sm btn-outline" onClick={handleSearchBySimilar}>
          Search by similar
        </button>
      </div>
    </Collapsible>
