...
export const Image: React.FC<ImageProps> = ({
  ...
  const Component =
    selectMode || (image as SimilarOSImage).similarity ? "button" : Link;
  const props = (() => {
    if (selectMode) {
      return {
  ...
  return (
    <Component
      {...(props as any)}
      className={twMerge(
        "w-full aspect-square rounded-lg bg-black/8 dark:bg-white/8",
        className,
      )}
    >
      <div
        className={twMerge("group block w-full h-full cursor-pointer relative")}
      >
