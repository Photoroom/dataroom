    ...
    <Collapsible name="details" title="Details">
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <p className="font-bold">{image ? defaultValue(formatNumber(image?.width, { decimals: 0 })) : <LoaderSkeleton />}</p>
        </div>
        <div>
          <p className="opacity-80">Height</p>
          <p className="font-bold">{image ? defaultValue(formatNumber(image?.height, { decimals: 0 })) : <LoaderSkeleton />}</p>
        </div>
        <div>
          ...
          <p className="font-bold" title={String(image?.aspect_ratio)}>
            {
              image ? (
                <>
                  {defaultValue(formatNumber(image?.aspect_ratio, { decimals: 6 }))} ({defaultValue(image?.aspect_ratio_fraction)})
                </>
              ) : <LoaderSkeleton />
            }
          </p>
