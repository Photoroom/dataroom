...
const TokenForm: React.FC<TokenFormProps> = ({ token, refetchTokens }) => {
  ...
  const handleChangeReadOnly = (checked: boolean) => {
    updateToken({id: token.id, data: {is_readonly: checked}}, {
      onSuccess: () => {
        refetchTokens();
...
export const SettingsPage: React.FC = function () {
  ...
  const cardClass = "mx-3 p-8 rounded-lg shadow-lg border border-light-300 dark:border-dark-300 bg-light-100 dark:bg-dark-100 flex flex-col gap-2";
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className={cardClass}>
        ...
        <div className="flex flex-col gap-2">
          ...
          <button type="button" className="btn btn-outline btn-sm shrink-0 self-start"onClick={handleCreateToken} disabled={isCreatingToken}><PlusIcon /><span>Create token</span></button>
        </div>
      </div>
      ...
      <div className={cardClass}>
        ...
        <div className="">
          {user.isStaff && urls.adminBackend && (
            <MenuLink href={urls.adminBackend}>Admin Backend</MenuLink>
          )}
